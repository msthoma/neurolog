...
if __name__ == '__main__':
    ...
    for i, (train_dataset, test_datasets) in enumerate(test_results.items()):
        for j, (test_dataset, results) in enumerate(test_datasets.items()):
            # here calculate statistics for accuracy for the last 5 iterations across all different versions of systems
            last_5 = results.loc[results.index > 8600].to_numpy()
            stats_table.append(
                [train_dataset, test_dataset, round(last_5.mean(axis=(0, 1)), 1), round(last_5.std(axis=(0, 1)), 1)])

            # calculate statistics for accuracy over the different runs for plots
            results = pd.concat([results.median(axis=1), results.min(axis=1), results.max(axis=1)], axis=1)
            results.columns = ["median", "min", "max"]

            color = next(colors)
            axes[i, j].set_ylim(40, 102)
