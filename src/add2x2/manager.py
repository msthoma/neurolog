...
class Add2x2Example(Example):
    ...
    def __str__(self):
        return 'digit11_image=' + str(self.digit11_image) + ',' + 'digit12_image=' + str(self.digit12_image) + ',' + \
               'digit21_image=' + str(self.digit21_image) + ',' + 'digit22_image=' + str(
            self.digit22_image) + ',label=' + str(self.label)


class Add2x2Manager(BenchmarkManager):

    def __init__(self):
        BenchmarkManager.__init__(self)

    def parseString(self, line: str) -> Add2x2Example:
        ...
        index2 = line.find(',row1_sum=', 0)
        digit22_str = line[index1 + len('digit22_image='):index2]
        digit22 = int(digit22_str)

        index1 = line.find('row1_sum=', 0)
        index2 = line.find(',row2_sum=', 0)
        row1_str = line[index1 + len('row1_sum='):index2]
        row1 = int(row1_str)

        index1 = line.find('row2_sum=', 0)
        index2 = line.find(',col1_sum=', 0)
        row2_str = line[index1 + len('row2_sum='):index2]
        row2 = int(row2_str)

        index1 = line.find('col1_sum=', 0)
        index2 = line.find(',col2_sum=', 0)
        col1_str = line[index1 + len('col1_sum='):index2]
        col1 = int(col1_str)

        index = line.find('col2_sum=', 0)
        col2_str = line[index + len('col2_sum='):]
        col2 = int(col2_str)

        return Add2x2Example(digit11, digit12, digit21, digit22, [row1, row2, col1, col2])

    def computeNeuralInput(self, example: Add2x2Example, dataset='train'):
        if dataset == 'train':
            datasets = [mnist_train_data] * 4
        else:
            datasets = [mnist_test_data] * 4
        return computeNeuralInput(
            [example.digit11_image, example.digit12_image, example.digit21_image, example.digit22_image], datasets)

    def computeGoal(self, example: Add2x2Example) -> Goal:
        return Add2x2Goal(example.label)
