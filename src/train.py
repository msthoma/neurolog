...
from src.loss import computeTensorWMC
from src.params import useGPU, logs_root


class Trainer(object):

    def __init__(self, network: nn.Module, outputClasses: List, benchmarkManager, optimizer: Optimizer, translator,
                 abduction, model_path: str):
    ...
    def train(self, examples: List, nr_epochs, minibatch_size=50, log_iter=100, snapshot_iter=None,
              snapshot_name='model', shuffle=True, neural_guided_abduction=False, logs_path=None):
        ...
        for epoch in range(nr_epochs):
            ...
            for index in indices:
                ...
                if snapshot_iter and iterations % snapshot_iter == 0:
                    fname = '{}_samples_{}_iter_{}_epoch_{}.mdl'.format(snapshot_name, len(examples), iterations,
                                                                        epoch + 1)
                    self.saveState(fname)

                if iterations % log_iter == 0:
                    f.write('{}\t{}\n'.format(iterations, time.time() - start))
                    print('Iteration: ', iterations, '\tAverage Loss: ', total_loss / log_iter, '\tTime',
                          time.time() - start)
