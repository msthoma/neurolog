...
from src.datasets import hasy_test_data, hasy_train_data
...
class Apply2x2Goal(Goal):
    ...
    def __str__(self):
        return 'digit1=' + str(self.digit1) + ',' + 'digit2=' + str(self.digit2) + ',' + 'digit3=' + str(
            self.digit3) + ',' + 'label=' + str(self.label)


class Apply2x2Example(Example):
    def __init__(self, digit1: int, digit2: int, digit3: int, op11_image: int, op12_image: int, op21_image: int,
                 op22_image: int, label: List):
...
class Apply2x2Manager(BenchmarkManager):
    ...
    def parseString(self, line: str) -> Apply2x2Example:
        ...
        return Apply2x2Example(digit1, digit2, digit3, op11_image, op12_image, op21_image, op22_image,
                               [row1, row2, col1, col2])
    ...
    def computeNeuralInput(self, example: Apply2x2Example, dataset='train'):
        ...
        return computeNeuralInput([example.op11_image, example.op12_image, example.op21_image, example.op22_image],
                                  datasets)
